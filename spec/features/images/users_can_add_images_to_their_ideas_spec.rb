  ...
  context "when a user visits their own homepage" do
    context "they fill out the form to create a new idea" do
      it "allows them to add images to their idea" do
        ...
        image = Image.create(name: "MetalMask", url: "https://res.cloudinary.com/tsl/image/upload/c_scale,f_auto,q_auto:good,w_680/960_b7nbge")
        ...
        select("MetalMask", from: "idea_image_ids").select_option
        click_on "Create Idea"

        expect(page).to have_content("MetalMask")
      end
    end
  end
end
